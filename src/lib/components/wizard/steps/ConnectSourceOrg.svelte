<script lang="ts">
	...
	let orgType = $state<'production' | 'sandbox' | 'developer' | 'scratch'>('production');
	let apiVersion = $state('60.0');

	const isConnected = $derived(wizardStore.state.sourceOrg.isConnected);
	const isConnecting = $derived(wizardStore.state.sourceOrg.isConnecting);
	const error = $derived(wizardStore.state.sourceOrg.error);
	const connectedOrg = $derived(wizardStore.state.sourceOrg.org);

	// Mock connection function - in real implementation, this would use OAuth
	async function handleConnect() {
		...
		try {
			...
			wizardStore.setSourceOrg(org, 'mock-access-token', instanceUrl);
		} catch (err) {
	...
	{:else}
		...
		<div class="space-y-4">
			...
			<div class="text-sm text-muted-foreground">
				<p>
					<strong>Note:</strong> In a production environment, this would use OAuth 2.0 to securely
					authenticate with Salesforce. For this demo, we're using a simplified connection flow.
				</p>
			</div>
		</div>
	{/if}
